...
class AddUser:
    ...
    def push_response_to_db(self):
        ...
        self.response_id = mycursor.lastrowid
    ...
    def print_response(self):
        response_dict = {}
        mycursor = mydb.cursor()
        get_response = "Select * from Response_Info where id = %s"
        data_response = (self.response_id,)
        mycursor.execute(get_response, data_response)
        result = mycursor.fetchone()
        if result[2] == "Success":
            response_dict["Request_id"] = result[1]
            response_dict["Response"] = result[2]
        elif result[2] == "Failure":
            response_dict["Request_id"] = result[1]
            response_dict["Response"] = result[2]
            response_dict["Reason"] = result[3]

        json_dump = json.dumps(response_dict)
        json_object = json.loads(json_dump)

        with open("responses.txt", mode="a", encoding="UTF-8") as response_file:
            response_file.write(str(json_object))
            response_file.write("\n")
            response_file.close()
